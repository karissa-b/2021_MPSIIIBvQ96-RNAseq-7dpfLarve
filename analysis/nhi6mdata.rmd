...
```{r meta}
meta <-  getGEO("GSE149149")
meta <- phenoData(meta$GSE149149_series_matrix.txt.gz) %>% 
 pData %>% 
  as_tibble

meta2 <- read_delim("data/Nhi_data/SraRunTable (1).txt", delim = ",")

meta2 %<>% 
  dplyr::select(geo_accession = `GEO_Accession (exp)`, everything()) %>% 
  left_join(meta) %>% 
  dplyr::select(SRR = Run,  oxygen_condition, 
                sample = title, 
                age = characteristics_ch1.2, 
                genotype = characteristics_ch1.3
                ) %>% 
  mutate(
    age = case_when(
      grepl(age, pattern = "6") ~ 6, 
      grepl(age, pattern = "24") ~ 24), 
    genotype = case_when(
      grepl(genotype, pattern = "Q96_K97del/+") ~ "Q96_K97del/+", 
      TRUE ~ "wt"), 
    genotype = factor(genotype, levels = c("wt", "Q96_K97del/+"))
  )
...
readTotals(fastqc_raw) %>%
  mutate(SRR = str_remove(Filename, ".fastq.gz")) %>% 
  left_join(meta2) %>%
  ggplot(aes(x = SRR, y = Total_Sequences)) +
  geom_col(position = "dodge") +
  coord_flip() +
  scale_fill_viridis_d(end = 0.8) +
  facet_wrap(~genotype+age+oxygen_condition, scales = "free_y", ncol = 1, strip.position = "right")
```
...
```{r}
fastqc_trim <- list.files(path = "data/Nhi_data/fastqc_trim",
  pattern = "zip",
  recursive = TRUE,
  full.names = TRUE) %>%
  FastqcDataList()
...
fastqc_align <- list.files(
  path = "data/Nhi_data/fastqc_align",
  pattern = "zip",
  recursive = TRUE,
  full.names = TRUE) %>%
  FastqcDataList()
...
```{r}
list.files("data/Nhi_data/starlog", full.names = TRUE) %>%
...
readTotals(fastqc_align) %>%
  mutate(SRR = str_remove(Filename, ".Aligned.sortedByCoord.out.bam")) %>%
  left_join(meta2) %>%
  ggplot(aes(x = SRR, y = Total_Sequences)) +
  geom_col(position = "dodge") +
  coord_flip() +
...
```{r}
FC_summary <- read.delim("data/Nhi_data/04_featureCounts/counts.out.summary")

colnames(FC_summary) %<>%
  str_remove(pattern = "X03_alignstar.bam.") %>%
  str_remove(pattern = ".Aligned.sortedByCoord.out.bam")

FC_summary %>%
 gather(key = "SRR", value = "NumReads", starts_with("SRR")) %>%
  left_join(meta2) %>%
  as_tibble() %>%
  dplyr::filter(NumReads > 0) %>%
  ggplot(aes(y = SRR, x = NumReads, fill = Status)) +
  geom_col() +
  scale_fill_viridis_d(end = 0.8) +
...
```{r}
featureCounts <- 
  read_delim("data/Nhi_data/04_featureCounts/counts.out", delim = "\t", skip = 1) %>%
  set_names(basename(names(.))) %>% 
  set_names(names(.) %>% str_remove(pattern = ".Aligned.sortedByCoord.out.bam")) %>% 
  as_tibble() %>% 
  dplyr::select(-c(Chr, Start, End, Length, Strand)) %>% 
  gather(key = "SRR", value = "counts", starts_with("SRR")) %>% 
  left_join(meta2) %>%
...
Dispersions were calculated across all samples. But then were filtered only to contain the 6m normoxia samples
...
samps2keep <- meta2 %>% 
  dplyr::filter(oxygen_condition == "normoxia") %>% 
  dplyr::filter(age == 6) %>% 
  .$sample

x.nhi <- x.nhi[,samps2keep]
```
...
q96_6_0_4 seems like an outlier. Might omit it

```{r}
moresams2keep <- x.nhi$samples %>% 
  dplyr::filter(sample != "q96_6_0_4_reanalyzed") %>% 
  .$sample

x.nhi <- x.nhi[,moresams2keep]
```


# GC Length check
```{r}
design <- model.matrix(~genotype, data = x.nhi$samples) %>% 
  set_colnames(gsub(colnames(.), pattern = "genotype", replacement = ""))

fit_1 <- x.nhi %>% 
  glmFit(design)

toptable_1 <- design %>% colnames() %>% .[2] %>%
  sapply(function(x){
    glmLRT(fit_1, coef = x) %>%
      topTags(n = Inf) %>%
      .[["table"]] %>%
...
x.nhi$offset <- cqn$glm.offset

# fit the glm
fit_cqn <- x.nhi %>% 
  glmFit(design)

toptables_cqn <- design %>% colnames() %>% .[2] %>% 
  sapply(function(x){
...
KEGG <- msigdbr("Danio rerio", category = "C2", subcategory = "CP:KEGG") %>% 
  left_join(x.nhi$genes, by = c("gene_symbol" = "gene_name")) %>% 
  dplyr::filter(gene_id %in% rownames(x.nhi)) %>% 
...
png("output/plots4poster/KEGG_oxphos.png", width = 6, height = 10, units = "cm", res = 300)
toptables_cqn %>% 
  bind_rows() %>% 
...
hmp.fry %>% 
  ...
  ggplot(aes(y = reorder(pathway, -harmonic_p),
             x = -log10(harmonic_p))) +
  geom_col(aes(), 
           position = "dodge")
...
# check the tf and ferritin genes
```{r}
iron_genes <- x.nhi$genes %>% 
  dplyr::filter(grepl(description, pattern = "transferrin|ferri")) %>% 
  .$gene_id
...
logCPM[iron_genes,] %>%
  ...
  gather(key = "sample", value = "logCPM", 2:9) %>% 
  left_join(x.nhi$samples) %>% 
  left_join(x.nhi$genes) %>% 
  ggplot(aes(x = genotype, y = logCPM)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter() +
  stat_mean(geom = "bar", colour = "black", fill = NA) +
...
mu <- x.nhi$counts %>%
  as.data.frame() %>%
  .[grepl(colnames(x.nhi$counts), pattern = "wt")] %>%
  rowMeans()

disp <-  x.nhi$tagwise.dispersion

fc <- function(y){exp(rnorm(y, log(1), 0.5*log(2)))}

power <- ssizeRNA_vary(nGenes = dim(x.nhi)[1], # Num detectable genes
...
## Data export
```{r}
x.nhi %>% saveRDS("data/R_objects/adult_brain/dgeNhi.rds")
logCPM %>% saveRDS("data/R_objects/adult_brain/logcpmNhi.rds")
toptables_cqn %>% saveRDS("data/R_objects/adult_brain/toptablescqnNhi.rds")
hmp.fry %>% saveRDS("data/R_objects/adult_brain/hmp_keggNhi.rds")
hmp.ire %>% saveRDS("data/R_objects/adult_brain/hmp_ireNhi.rds")
celltype %>% saveRDS("data/R_objects/adult_brain/celltypeNhi.rds") 
fry_GO %>% saveRDS("data/R_objects/adult_brain/fryGoNhi.rds") 
```
